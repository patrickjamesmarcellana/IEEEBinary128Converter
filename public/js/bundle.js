...
exports.loadBinaryString = loadBinaryString;
exports.loadDecimalString = updateFromNewDecimalString;
exports.toBinary16 = void 0;
var _decimal = _interopRequireDefault(require("decimal.js"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
_decimal.default.set({
  precision: 150
});
_decimal.default.rounding = _decimal.default.ROUND_DOWN;
const Binary16 = {
  TOTAL_BITS: 16,
  EXPONENT_BITS: 5,
  MANTISSA_BITS: 10,
  ...
  return {
    ...
    pack: function () {
      ...
      if (this.exponent < number_format.MIN_NORMALIZED_EXPONENT) {
        ...
        this.mantissa = Array(zeroes_to_add_to_start).fill(0).concat(this.mantissa);
      ...
      if (this.mantissa[0]) {
        ...
        console.assert(1 <= e_prime && e_prime <= number_format.INFINITY_E_PRIME, "exponent does not fit in range");
      } else {
        // denormalized
        e_prime = 0;
      }
      var exponent_binary = getNumberBinary(new _decimal.default(e_prime));
      exponent_bits = Array(number_format.EXPONENT_BITS - exponent_binary.length).fill(0).concat(exponent_binary);
      console.assert(exponent_bits.length === number_format.EXPONENT_BITS, `exponent bits length: expected ${number_format.EXPONENT_BITS}, got`, exponent_bits.length);
      console.log(exponent_bits.join(""));
      mantissa_bits = addZerosToArray(this.mantissa.slice(1), number_format.MANTISSA_BITS);
      console.assert(mantissa_bits.length === number_format.MANTISSA_BITS, `mantissa bits length: expected ${number_format.MANTISSA_BITS}, got`, mantissa_bits.length);
      console.log(addSpaces(mantissa_bits.join("")));
      binary_val = sign_bit.join("") + exponent_bits.join("") + mantissa_bits.join("");
      console.assert(binary_val.length === number_format.TOTAL_BITS, `binary value length: expected ${number_format.TOTAL_BITS}, got`, binary_val.length);
...
function updateFromNewDecimalString(decimal_string, exponent) {
  ...
  var x = new _decimal.default(decimal_val);
  if (decimal_val[0] !== '-') {
    sign_bit = [0];
  } else {
    sign_bit = [1];
  ...
  var [fractional_part, removed_zeros, remaining] = getDecimalBinary(x, 113 - integer_part.length, x.lessThan(1) /* if 0.xxx, then remove leading zeros in decimal part */);
  var ZERO_POINT_FIVE = new _decimal.default("0.5");
  var number = mantissa_exponent_pair(integer_part.concat(fractional_part), integer_part.length > 0
...
function getError() {
  ...
  var decimal = new _decimal.default(decimal_val);
  var actual = new _decimal.default(actual_val);
  error_val = actual.sub(decimal).toFixed();
}
function binaryToFractional(string) {
  // Gets the fractional value of a given string.

  var decimal = new _decimal.default(0);
  var one = new _decimal.default(1);
  var power = new _decimal.default(2);
...
function binaryToNumber(string) {
  ...
  var decimal = new _decimal.default(0);
  var power = new _decimal.default(1); // The power of 2 associated with the rightmost bit
...
$("#submit-btn").click(e => {
  ...
  if ($("#binary-input").hasClass("active")) {
    result = (0, _converter.loadBinaryString)(numberValue, exponentValue);
  } else {
    result = (0, _converter.loadDecimalString)(numberValue, exponentValue);
  }
  const first16Bits = result.bin.substring(0, 16);
  const first16BitsGrouped = first16Bits.match(/.{1,4}/g).join(" ");
  const significandBitsGrouped = result.bin.substring(16).match(/.{1,4}/g).join(" ");
